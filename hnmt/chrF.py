...
# adjusted for hnmt by Jörg Tiedemann
...
def f1(correct, total_hyp, total_ref, max_length, beta=3, smooth=0):
    ...
    if f:
        f = f / ((beta**2 * precision) + recall)
    return f, precision, recall


def chrF(references, candidates, max_length=6, spaces=False, beta=3):
    correct = [0]*max_length
    total = [0]*max_length
    total_ref = [0]*max_length
    for line, line2 in zip(references,candidates):

      ngrams_ref = extract_ngrams(line, max_length=max_length, spaces=spaces)
      ngrams_test = extract_ngrams(line2, max_length=max_length, spaces=spaces)
    ...
    return f1(correct, total, total_ref, max_length, beta)
